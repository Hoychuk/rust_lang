...
impl ChatServer {
    ...
    async fn handle_connection(&self, stream: TcpStream) {
        ...
        Some(msg) = read.next() => {
            ...
            match serde_json::from_str::<serde_json::Value>(&raw_msg) {
                Ok(json) => {
                    if json["type"] == "file" {
                        println!("Received file: {}", json["filename"]);
                        // Можна зберігати файл на сервері або пересилати іншим користувачам
                        let _ = self.tx.send(Message {
                            from: json["from"].as_str().unwrap().to_string(),
                            content: format!("File: {}", json["filename"].as_str().unwrap()),
